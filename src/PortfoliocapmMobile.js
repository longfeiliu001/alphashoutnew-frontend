...
export default function AlphashoutPortfolioAnalyticsMobile() {
  ...
  // Risk-free and benchmark search states
  const [riskFreeModalVisible, setRiskFreeModalVisible] = useState(false);
  const [riskFreeSearchQuery, setRiskFreeSearchQuery] = useState('');
  const [riskFreeSearchResults, setRiskFreeSearchResults] = useState([]);
  ...
  // API search function
  const searchSymbols = async (query, setResults, setLoading) => {
    if (query.length < 2) {
      setResults([]);
      return;
    }
    ...
    try {
      ...
      if (response.data.success && response.data.bestMatches) {
        const results = response.data.bestMatches.slice(0, 5);
        setResults(results);
      } else {
        setResults([]);
      }
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (searchQuery.trim()) {
        searchSymbols(searchQuery.trim(), setSearchResults, setIsSearching);
  ...
  // Search debounce for risk-free symbol
  useEffect(() => {
    const timer = setTimeout(() => {
      if (riskFreeSearchQuery.trim()) {
        searchSymbols(riskFreeSearchQuery.trim(), setRiskFreeSearchResults, setIsRiskFreeSearching);
      } else {
        setRiskFreeSearchResults([]);
      }
    }, 500);
  ...
  // Search debounce for benchmark symbol
  useEffect(() => {
    const timer = setTimeout(() => {
      if (benchmarkSearchQuery.trim()) {
        searchSymbols(benchmarkSearchQuery.trim(), setBenchmarkSearchResults, setIsBenchmarkSearching);
      } else {
  ...
  const sendToWebhook = async () => {
    if (!isAuthenticated) {
      message.warning('Please login to perform analysis');
      return;
    }

    if (availableQuota < quotaConfig.ANALYSIS_TOKEN_FEE) {
      message.error(`Insufficient quota! Need ${quotaConfig.ANALYSIS_TOKEN_FEE} tokens`);
      return;
    }
    ...
    setIsSending(true);
    setGlobalPortfolioLoadingState(true);
    setWebhookResponse(null);
    clearPortfolioAnalysisResult();
    setGlobalPortfolioError(null);
    ...
    try {
      ...
      if (response.data.success) {
        ...
        updatePortfolioAnalysisResult(newWebhookResponse);
        setAvailableQuota(response.data.quota.remaining);
        setResultsModalVisible(true);
        
        message.success(`Analysis complete! ${response.data.quota.remaining} tokens remaining`);
      } else {
  ...
  return (
    <div style={{ 
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <Text style={{ fontSize: '12px', color: AlphaShout_COLORS.gray }}>
              {isAuthenticated ? `${availableQuota} tokens` : 'Not logged in'}
            </Text>
          </div>
          <Space>
            <Button 
      ...
      <Card 
        ...
        extra={
          <Button 
            ...
            size="small" 
            icon={<PlusOutlined />}
            onClick={() => setSymbolsModalVisible(true)}
          >
            Add
          </Button>
        }
      ...
      <Button 
        ...
        disabled={!isAuthenticated || availableQuota < quotaConfig.ANALYSIS_TOKEN_FEE}
        style={{ 
          height: '48px',
          fontSize: '16px',
          marginBottom: '12px'
      ...
      {/* Risk-Free Symbol Search Modal */}
      <Modal
        title="Search Risk-Free Asset"
        visible={riskFreeModalVisible}
        onCancel={() => {
          setRiskFreeModalVisible(false);
          setRiskFreeSearchQuery('');
          setRiskFreeSearchResults([]);
        }}
        footer={null}
        width="90%"
      ...
      >
        <Input
          ...
          placeholder="Search risk-free assets (e.g., BND, TLT)..."
          value={riskFreeSearchQuery}
          onChange={(e) => setRiskFreeSearchQuery(e.target.value)}
          style={{ marginBottom: '12px' }}
        />
        
        {riskFreeSearchResults.length > 0 && (
          <div style={{ marginBottom: '12px', maxHeight: '300px', overflowY: 'auto' }}>
            {riskFreeSearchResults.map((result, index) => (
              <div
                key={index}
                onClick={() => {
                  updateConfigField('risk_free_symbol', result['1. symbol']);
                  setRiskFreeModalVisible(false);
                  setRiskFreeSearchQuery('');
                  setRiskFreeSearchResults([]);
                  message.success(`Risk-free asset set to ${result['1. symbol']}`);
                }}
                style={{
                  padding: '10px',
                  cursor: 'pointer',
        ...
        {/* Common risk-free suggestions */}
        <div style={{ marginTop: '16px' }}>
          <Text style={{ fontSize: '12px', color: AlphaShout_COLORS.gray }}>Common Risk-Free Assets:</Text>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginTop: '8px' }}>
            {['BND', 'TLT', 'IEF', 'SHY', 'AGG', 'GOVT'].map(symbol => (
              <Tag
                key={symbol}
                style={{ cursor: 'pointer' }}
                color="blue"
                onClick={() => {
                  updateConfigField('risk_free_symbol', symbol);
                  setRiskFreeModalVisible(false);
                  message.success(`Risk-free asset set to ${symbol}`);
                }}
              >
                {symbol}
              </Tag>
      ...
      {/* Benchmark Symbol Search Modal */}
      <Modal
        title="Search Market Benchmark"
      ...
      >
        <Input
          ...
          placeholder="Search benchmarks (e.g., SPY, QQQ)..."
        ...
        {benchmarkSearchResults.length > 0 && (
          <div style={{ marginBottom: '12px', maxHeight: '300px', overflowY: 'auto' }}>
            {benchmarkSearchResults.map((result, index) => (
              <div
                key={index}
                onClick={() => {
                  updateConfigField('market_benchmark_symbol', result['1. symbol']);
                  setBenchmarkModalVisible(false);
                  setBenchmarkSearchQuery('');
                  setBenchmarkSearchResults([]);
                  message.success(`Benchmark set to ${result['1. symbol']}`);
                }}
                style={{
                  padding: '10px',
                  cursor: 'pointer',
        ...
        {/* Common benchmark suggestions */}
        <div style={{ marginTop: '16px' }}>
          <Text style={{ fontSize: '12px', color: AlphaShout_COLORS.gray }}>Common Benchmarks:</Text>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginTop: '8px' }}>
            {['SPY', 'QQQ', 'DIA', 'IWM', 'VTI', 'VOO', 'VT', 'EFA'].map(symbol => (
              <Tag
                key={symbol}
                style={{ cursor: 'pointer' }}
                color="green"
                onClick={() => {
                  updateConfigField('market_benchmark_symbol', symbol);
                  setBenchmarkModalVisible(false);
                  message.success(`Benchmark set to ${symbol}`);
                }}
              >
                {symbol}
              </Tag>
        ...
        <Form layout="vertical">
          ...
          <Form.Item label="Risk Free Symbol">
            <Space.Compact style={{ width: '100%' }}>
              ...
              <Button 
                icon={<SearchOutlined />}
                onClick={() => setRiskFreeModalVisible(true)}
              >
                Search
              </Button>
            </Space.Compact>
